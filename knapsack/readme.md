...
`bb_bs2.cpp` includes some minor changes to `bb_bs.cpp` to make it perform better on correlated data, skipping identical items (same weight and value) in the exclusion branch. `bb_bs2.cpp` also tries to tighten the initial lower bound by filling in the rest of the max capacity with items, and uses the floor of the fractional knapsack solution as the upper bound.
